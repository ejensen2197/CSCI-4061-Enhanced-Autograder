...
static void RedirectSolnOutput(SolnDataT *SolnData)
{
    /**
     * TODO => Change C:
     *      + Open the output file and redirect STDOUT for the child process using dup2
...
static void NO_RETURN RunSoln_Exec(SolnDataT *SolnData)
{
    /**
     * TODO => Change B:
     *      + Nothing to change in this function, if RedirectSolnOutput is implemented correctly,
     *        the remaining code is provided for you... This is essentially project #1
     */
    ...
    execl(SolnData->ExePath, SolnData->ExeName, ipc_type_param, input_param, NULL);

    fprintf(stderr, "ERROR: Execl failed... Path [%s] Exe [%s] IpcType [%s] Param [%s]\n", SolnData->ExePath, SolnData->ExeName, ipc_type_param, SolnData->InputParamStr);
    exit(EXIT_FAILURE);
}
...
static void NO_RETURN RunSoln_Pipe(SolnDataT *SolnData)
{
    ...
    /* Convert the read end of the pipe file descriptor as a string to pass into the soln executable */
    ...
    /* Exec the solution with the parameter set to the file descriptor of the read end of the pipe */
    execl(SolnData->ExePath, SolnData->ExeName, ipc_type_param, pipe_read_end_param, NULL);

    /* Error check execl call */
    fprintf(stderr, "ERROR: Execl failed... Path [%s] Exe [%s] IpcType [%s] Param [%s]\n", SolnData->ExePath, SolnData->ExeName, ipc_type_param, pipe_read_end_param);
    exit(EXIT_FAILURE);
}
...
static void StuckProcessTimerHandler(int signum)
{
    /**
     * TODO => Change D:
     *      + This is the timer handler function... once it expires you should loop the SolnDataArr.Array
     *      + Any process that still has a WaitStatus of IN_PROGRESS_WAIT_STATUS should be killed with the kill system call
...
static void StartStuckProcessTimer(void)
{
     ...
     *      + Use STUCK_PROC_TIMER_VALUE_SEC and STUCK_PROC_TIMER_VALUE_USEC for the timer values
     * 
     * TODO IMPORTANT =>
...
static int GetBatchSize(void)
{
    ...
    /* Iterate over the lines of the file, if the string "processor" is contained this is a CPU */
    cpu_count = 0;
 ...
 * Function:    StartBatch
 * 
 * Description: StartBatch will create a child process for each solution to run in the batch.
 *              It will then use the IPCType to call a particular starting function for that solution.
 * 
 * Parameters:
...
static void StartBatch(IPCTypeT IPCType)
{
    ...
    {
        SolnDataT *cur_soln_dat = &SolnDataArr.Array[CurrentBatchData.StartIndex + i];

        /* Set the solutions wait status to in-progress prior to forking and exec'ing */
 ...
 * Function:    BatchRun
 * 
 * Description: BatchRun will split the solution data array into batch size chunks 
 *              and call StartBatch then WaitBatch on each batch started
 * 
 * Parameters:
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    PopulateSolnDataArr(
        ...
        SolnDataArr.Array,            /* Allocated array where the solution data should be populated */
